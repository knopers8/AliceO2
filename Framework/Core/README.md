...
`TRACKS` and `CLUSTERS`, for any given time interval both needs to be produced
before the computation can happentime interval both needs to be produced before
the computation can happen.
...
```C++
customize(std::vector<CompletionPolicy> &policies);
```
