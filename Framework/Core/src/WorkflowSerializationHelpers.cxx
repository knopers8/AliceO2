  ...
  for (auto& processor : workflow) {
    ...
    for (auto& output : processor.outputs) {
      ...
      if (output.binding.value.empty()) {
        auto autogenerated = output.origin.as<std::string>() + "-" + output.description.as<std::string>() + "-" + (output.subSpec ? std::to_string(output.subSpec.value()) : "*");
