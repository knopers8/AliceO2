...
void
WorkflowHelpers::injectServiceDevices(WorkflowSpec &workflow) {
    ...
    for (size_t ii = 0; ii < consumer.inputs.size(); ++ii) {
      ...
      switch(input.lifetime) {
        case Lifetime::Condition:
          ccdbBackend.outputs.push_back(output);
          break;
        case Lifetime::QA:
          qaStore.outputs.push_back(output);
          break;
        case Lifetime::Timer:
          timer.outputs.push_back(output);
          break;
        case Lifetime::Transient:
          transientStore.outputs.push_back(output);
