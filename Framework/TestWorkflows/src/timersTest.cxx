...
//          sleep(rand() % 2);
  ...
  DataProcessorSpec accumulator{
    ...
    AlgorithmSpec{
      (AlgorithmSpec::InitCallback) [](InitContext& ictx) {
        ...
        auto bookTimesliceCallback = [&timesliceIndex = ictx.services().get<TimesliceIndex>(),
          ...
          TimesliceId tid{duration_cast<milliseconds>(timeNow.time_since_epoch()).count()};
          if (duration_cast<milliseconds>(timeNow - *time).count() > 5000 ) {

            LOG(INFO) << "tid.value " << tid.value << ", timeNow " << duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count();

            for (size_t i = 0; i < timesliceIndex.size(); ++i) {
              TimesliceSlot slot{ i };
              if (timesliceIndex.isValid(slot) == false) {
                continue;
              }
              if (timesliceIndex.getTimesliceForSlot(slot).value == tid.value) {
            ...
            *time = timeNow;
            ...
            auto slot = timesliceIndex.replaceLRUWith(newContext);
            timesliceIndex.associate(tid, slot);
          }
        };
    ...
    Options{
      ConfigParamSpec{ "period-timer", VariantType::Int, 5000000, { "timer period" }}
    }
  };
