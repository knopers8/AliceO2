        ...
        for nb_dispatchers in ${number_of_dispatchers[@]}; do
          for ((rep=0;rep<repetitions;rep++)); do
            ...
            while [ "$messages_per_second" == 'error' ] || [ -z "$messages_per_second" ]; do
              ...
              timeout -k 60s $test_duration_timeout /home/pkonopka/alice/sw/BUILD/O2-latest/O2/stage/bin/o2-datasampling-datasampling-benchmark $common_args --payload-size $payload_size --producers $nb_producers --dispatchers 1 --sampling-fraction $fraction > "$run_log"
